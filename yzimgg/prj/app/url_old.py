...
"""
    ...
    if attrs.has_key('qqtotalparts') and int(attrs['qqtotalparts']) > 1:
        ...
        yzfile_folder = "d:/fmapp/YZimgG/prj/static/media"
        yzfilename = 'test.jpg'
        dest1 = os.path.join(yzfile_folder, yzfilename)
...
def uploadinfo():
    ...
    requests.post("http://127.0.0.1:5002/gemo/updataTK", data={"key"    :json.dumps({"spcode":code}), "setvalue":json.dumps({"filename":filename})})
    return 'ok'
...
"""
        ...
        if validate(request.form):
            ...
            try:
                yzfilename = request.form['spcode']             
                return make_response(200, { "success": True})  #for fine_upload
            except:
                return make_response(200, [{ "success": True, "error":"", "name":"fengmu.jpg", "size":"21.84kb" }])    #for jupload
        else:
